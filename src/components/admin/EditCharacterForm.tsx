...
interface Props {
  character: { _id: string; name: string; imageUrl: string; height?: number };
}

export default function EditCharacterForm({ character }: Props) {
  const [name, setName] = useState(character.name);
  const [imageUrl, setImageUrl] = useState(character.imageUrl);
  const [preview, setPreview] = useState("");
  const [height, setHeight] = useState(character.height ?? 160);
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm space-y-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-gray-200 disabled:text-gray-400 text-white font-black py-3 rounded-2xl text-lg transition-colors"
        >
