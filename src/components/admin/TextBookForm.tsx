...
import { createTextBook } from "@/app/admin/books/create/actions";
...
export default function TextBookForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // inject per-page text into FormData
    pages.forEach((text) => fd.append("page", text));
    startTransition(() => createTextBook(fd));
  };
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        {pages.map((text, i) => (
          <div key={i} className="bg-white rounded-3xl p-5 shadow-sm">
            ...
            <textarea
              ...
              placeholder={
                i === 0
                  ? "Enter Japanese text…\nEach line or sentence ending with 。！？ becomes a sentence.\nExample:\nきょうは いい てんきです。\nそとで あそびましょう！"
                  : "Continue the story on this page…"
              }
              rows={6}
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full bg-pink-500 hover:bg-pink-600 disabled:bg-pink-300 text-white font-black py-4 rounded-3xl text-xl transition-colors active:scale-95"
      >
        {isPending ? "Saving…" : "📚 Create Book"}
      </button>
    </form>
  );
}
