...
export default function SentenceExcelImport({ topicId }: { topicId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    fd.append("file", file);
    try {
      const res = await fetch(`/api/topics/${topicId}/sentences/import`, { method: "POST", body: fd });
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-black text-gray-700">📊 Import Sentences from Excel</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {loading ? "Importing…" : "📥 Import Sentences"}
        </button>
      </form>
        ...
        <div className={`rounded-xl p-3 ${result.success ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
          {result.error ? (
            ...
            <div className="space-y-1">
              <p className="font-black text-green-700">✅ Imported {result.inserted} sentences!</p>
