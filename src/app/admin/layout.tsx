      ...
      <div className="flex-1 bg-gray-50 overflow-auto">
        ...
        <div className="md:hidden flex gap-2 p-3 bg-gray-900 overflow-x-auto">
          {[
            ...
            { href: "/admin/dictionary/new", icon: "➕", label: "Add" },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
...
function NavLink({ href, icon, label }: { href: string; icon: string; label: string }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-300 hover:text-white hover:bg-gray-700 transition-all font-medium"
    >
