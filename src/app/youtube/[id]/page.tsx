...
export default function YoutubeStudyPage({ params }: { params: { id: string } }) {
  ...
  const lookupWord = useCallback((w: string) => {
    ...
    fetch(`/api/dictionary?q=${encodeURIComponent(w)}&exact=true&limit=1`)
      .then((r) => r.json())
      .then((data) => setDictEntry(data.words?.[0] ?? null))
            ...
            return (
              ...
              >
                <div className="flex flex-wrap gap-x-1.5 gap-y-1">
                  {line.text.split(/\s+/).filter(Boolean).map((tok, ti) => (
                    <span
                      key={ti}
                      onClick={(e) => { e.stopPropagation(); lookupWord(tok); }}
                      className="font-bold cursor-pointer rounded px-0.5 transition-all"
                      style={{
                        color: isActive ? "#fff" : "rgba(255,255,255,0.6)",
                        fontSize: 16,
                        background: word === tok ? "rgba(251,191,36,0.35)" : "transparent",
                      }}
                    >
                      {tok}
                    </span>
          ...
          {word && !lookupLoading && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-white font-black text-2xl">{word}</span>
                <button onClick={() => speak(word)} className="text-lg active:scale-90">🔊</button>
              </div>
              {dictEntry ? (
                <div className="space-y-1.5">
                  ...
                  {dictEntry.example_image_url && (
                    ...
                    <img src={dictEntry.example_image_url} alt={word} className="w-full rounded-xl mt-2 object-cover" style={{ maxHeight: 140 }} />
                  )}
                </div>
              ) : (
